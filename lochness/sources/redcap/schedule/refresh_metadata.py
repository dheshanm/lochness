#!/usr/bin/env python
"""
Scheduler task to refresh REDCap metadata for all active REDCap data sources.
"""
...
MODULE_NAME = "lochness.sources.redcap.schedule.refresh_metadata"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "e.g. --site_id_list site001 site002 site003"
        ),
        required=False,
        default=None,
    )
    ...
    script_path = (
        repo_root / "lochness" / "sources" / "redcap" / "tasks" / "refresh_metadata.py"
    )
    ...
    for site in project_sites:
        job = Job(
            job_payload=f"{scheduler_exec} {script_path} -p {site.project_id} -s {site.site_id}",
            job_metadata={
                "CWD": str(repo_root),
            },
            job_status="PENDING",
            job_tags=["redcap", "metadata_refresh", site.project_id, site.site_id],
